...
twin_t twinArray[] = {
	...
	{.twinKey = "fanspeed",.twinVar = &ltc1695_manual_value,.twinFd = NULL,.twinGPIO = NO_GPIO_ASSOCIATED_WITH_TWIN,.twinType = TYPE_INT,.active_high = true},
	{.twinKey = "fanmode",.twinVar = &ltc1695_mode,.twinFd = NULL,.twinGPIO = NO_GPIO_ASSOCIATED_WITH_TWIN,.twinType = TYPE_BOOL,.active_high = true}
};
...
// Call by fan driver when the condition change 
void updateFanDevice() {
	for (int i=4;i<=5;i++)
		checkAndUpdateDeviceTwin(twinArray[i].twinKey, twinArray[i].twinVar, twinArray[i].twinType, true);
}
...
void checkAndUpdateDeviceTwin(char* property, void* value, data_type_t type, bool ioTCentralFormat)
{
	...
	if (pjsonBuffer == NULL) {
		LogWebDebug("ERROR: not enough memory to report device twin changes.");
	}
	...
	if (property != NULL) {
		...
		if (nJsonLength > 0) {
			LogWebDebug("[MCU] Updating device twin: %s\n", pjsonBuffer);
		...
		if (json_object_has_value(desiredProperties, twinArray[i].twinKey) != 0)
		{
			...
			case TYPE_BOOL:
				*(bool*)twinArray[i].twinVar = (bool)json_object_get_boolean(currentJSONProperties, "value");
				...
				// Support for change AUTO/MANU mode 
				if (!strcmp(twinArray[i].twinKey,"fanmode")) {
					ltc1695_setmode( *(bool*)twinArray[i].twinVar);
			...
			case TYPE_FLOAT:
				*(float*)twinArray[i].twinVar = (float)json_object_get_number(currentJSONProperties, "value");
				LogWebDebug("Received device update. New %s is %0.2f\n", twinArray[i].twinKey, *(float*)twinArray[i].twinVar);
			...
			case TYPE_INT:
				*(int*)twinArray[i].twinVar = (int)json_object_get_number(currentJSONProperties, "value");
				LogWebDebug("Received device update. New %s is %d\n", twinArray[i].twinKey, *(int*)twinArray[i].twinVar);

				if (!strcmp(twinArray[i].twinKey, "fanspeed")) {
	
					ltc1695_manual_value = *(int*)twinArray[i].twinVar;
					if (!ltc1695_mode)
						ltc1695_set(*(int*)twinArray[i].twinVar,false);
				}
