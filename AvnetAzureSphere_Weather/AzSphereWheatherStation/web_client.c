                                                 ...
                                                 void *userData)
{
    ...
    if (block->data == NULL) {
        Log_Debug("ERROR: Out of memory, realloc returned NULL");
        abort();
    }
                              ...
                              void *socketUserData)
{
    ...
    if (eventsMask != 0) {
        ...
        if (res == -1) {
            Log_Debug("ERROR: Could not add or modify fd '%d' the epoll set", fd);
            return -1;
        }
    }
...
void WebClient_Fini(void)
{
    CurlFini();
		...
		CloseFdAndPrintError(curlTimerFd, "curlTimerFd");
}
